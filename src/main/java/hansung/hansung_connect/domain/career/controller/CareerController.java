...
public class CareerController {
    ...
    @Operation(
            summary = "커리어 추가",
            description = """
                    커리어를 하나 추가하는 API입니다.<br>
                    - startYm, endYm 형식: "yyyy-MM" 또는 "yyyy.MM"<br>
                    - 재직 중(employed == true)이면 endYm은 null이어야 합니다.<br><br>
                    JobType ENUM<br>
                    - PERMANENT(정규직), TEMPORARY(계약직), INTERN(인턴), FREELANCER(프리랜서)
                    """
    )
    @PostMapping("/careers")
    ...
    @Operation(
            summary = "커리어 여러 개 일괄 추가",
            description = """
                    온보딩 등에서 커리어를 여러 개 한 번에 등록하는 API입니다.<br>
                    - items 배열에 단건 생성과 동일한 스펙을 담아 전송합니다.<br>
                    - startYm, endYm 형식: "yyyy-MM" 또는 "yyyy.MM"<br>
                    - 재직 중(employed == true)이면 endYm은 null이어야 합니다.
                    """
    )
    ...
    @Operation(
            summary = "커리어 수정(전체 대체)",
            description = """
                    기존 커리어 정보를 전체 대체 방식으로 수정합니다.<br><br>
                    요청 규칙<br>
                    - careerId: 수정할 커리어 ID(PathVariable)<br>
                    - 본문 스펙은 생성 요청과 동일하며 전달된 값으로 전부 교체됩니다.<br>
                    - 재직 중(employed == true)이면 endYm은 null이어야 합니다.<br>
                    - 비재직(employed == false)이면 endYm은 필수입니다.<br>
                    - 연월 형식: "yyyy-MM" 또는 "yyyy.MM"
                    """
    )
